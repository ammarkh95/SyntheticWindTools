...
!    | |   | |   | || (\ (   | (  \ \ | | \_  )| (      | | \   |         !       //#\\
!    | |   | (___) || ) \ \__| )___) )| (___) || (____/\| )  \  |         !     // ### \\
!    )_(   (_______)|/   \__/|/ \___/ (_______)(_______/|/    )_)         !   //  #####  \\
!                                                                         !      #######
!                                                                         !      ### ###

!   Synthetic Turbulence Generation for Wind Field Simulations
...
! This program generates a 3D turbulent (flucutating) velocity field according to Mann uniform shear model and using Fourier analysis
! The model adopts von Kármán isotropic incompressible energy spectrum which is then modified according to the rapid distortion theory

! The input parameters for generating the turbulence box domain (with Cartesian equidistant grids):

! (NX,NY,NZ) No. of grids in each direction (needs to be power of 2 for Fast-Fourier transform)
! (DX,DY,DZ) Grids spacings in each direction
! (L) Turbulence integral length scale  
! (alpaeps) Scaling coefficent for controlling turbulence intensity
! (Gamma) Distortion factor controlling the trubulence anisotropy (shear), when (Gamma = 0) isotropic spectrum is obtained
! (randseed) seed number for random numbers generation sequence 

! The output files / diagnostics include :

! Three binary files (u,v,w) for streamwise, spanwise and vertical velocity components (values in (32-bit,float) precision)
! Varinace value of the generated stochastic velocity field along the three wind directions
! Cross correlations of the generated stochastic velocity field [rho12,rho13,rho23]

!############################################################################
! References:

! Mann, Jakob: Wind field simulation. In: Probabilistic Engineering Mechanics 13 (1998), October, Nr. 4, p. 269–282

! Andre, Michael S.: Aeroelastic Modeling and Simulation for the Assessment of Wind Effects on a Parabolic Trough Solar Collector,
! Technische Universität München, Ph.D.thesis, 2018

! Gilling, Lasse: Airfoils in Turbulent Inflow. Denmark, Ph.D. thesis, 2009

! Khallouf, A: Developments for Improved FEM-Based LES Simulations in Computational Wind Engineering, Technische Universität München,
...
!****************************************************************************
! Read input from file (input.inp)
!****************************************************************************
...
write(*,100) '+----------------------------------------------------------------+'
write(*,100) '| TurbGen Synthetic Wind Generator v 1.0.0 @ (TUM - STATIK)      |'
write(*,100) '+----------------------------------------------------------------+'  
write(*,100) '+----------------------------------------------------------------+'
write(*,100) '| Simulation of turbulent wind field has started                 |'
write(*,100) '+----------------------------------------------------------------+'
endif
! Start timer
...
enddo !j3
if(write_screen.eq.1)then
write(*,100) '| Fourier coefficients have been calculated                      |'
endif
!****************************************************************************
! Do the iFFT
!****************************************************************************
...
!****************************************************************************
100 format(A66)
101 format(A13,i11,A42)
102 format(A13,i11,A23,F17.6,A2)
103 format(A13,F15.6,A19,F17.6,A2)
104 format(A13,F15.6,A38)
105 format(A13,i11,A17,ES17.6,A8)
106 format(A41,ES17.6,A8)
107 format(A15,F17.6,A34)
! The end!
end program TurbGen
!****************************************************************************
! End program
